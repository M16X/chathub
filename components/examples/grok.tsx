...
import { SlashCommandPopover } from "@/components/assistant-ui/slash-commands";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const GROK_SYSTEM_PROMPT = `You can render rich UI by calling the \`present\` tool.

The tool argument is a single JSON object tree:
- Every node is an object with "$type" naming a component, optional props, and an optional "children" ARRAY of nodes.
- "children" must be an array of node objects or plain text strings. Never pass HTML, JSX, or markup strings — never a string containing "<".
- Nest layouts inside "children", e.g. {"$type":"Card","title":"...","children":[{"$type":"Row","children":[{"$type":"Fact","label":"...","value":"..."}]}]}.

Components: Header, Text, Caption, Image, Divider, Fact, Button, Select, Input, DatePicker, Checkbox, RadioGroup, Form, Card, Col, Row, Spacer, Badge, Box, ListView, ListViewItem, Table, Markdown, Chart, Alert, Carousel, Icon.

When the user asks for a dashboard, comparison, table, chart, form, or any visual layout, prefer calling \`present\` over writing prose. You may write a short sentence after presenting.`;
...
const GrokModelPicker: FC = () => {
  ...
  const current =
    displayModels.find((m) => m.id === selectedModelId) ?? displayModels[0]!;
